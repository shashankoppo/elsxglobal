    ...
    <section className="relative py-24 sm:py-32 overflow-hidden">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid lg:grid-cols-2 gap-6">
          <Link
            href="/blog"
            className="group glass-strong rounded-2xl p-8 hover:border-primary/40 transition-all duration-300 hover:-translate-y-1 flex flex-col"
          >
            <div className="flex items-center justify-between mb-5">
              <span
                className={`text-xs font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full ${CATEGORY_COLORS[FEATURED.category]}`}
              >
                {FEATURED.category}
              </span>
            ...
            <h3 className="text-2xl font-semibold mb-4 leading-tight group-hover:text-primary transition-colors">
              {FEATURED.title}
            </h3>
          ...
          <div className="space-y-4">
            {ARTICLES.map((a) => (
              <Link
                key={a.title}
                href="/blog"
                className="group glass rounded-2xl p-5 hover:border-primary/40 transition-all duration-300 block"
              >
                <div className="flex items-center justify-between mb-3">
                  <span
                    className={`text-[11px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full ${CATEGORY_COLORS[a.category]}`}
                  >
                    {a.category}
                  </span>
                  <span className="flex items-center gap-1 text-xs text-muted-foreground">
